    ...
    <section id="experience" className="py-24 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
          <div>
            <span className="font-mono text-xs uppercase tracking-[0.2em] text-primary">02 / Experience</span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white mt-2">Career History</h2>
          </div>
        ...
        <div className="space-y-8 max-w-4xl">
          {experiences.map((exp, idx) => (
            <motion.div
              ...
              key={idx}
              className="glass p-8 rounded-xl border border-white/5 hover:border-primary/20 hover:bg-white/[0.01] transition-all relative group"
            >
              {/* Card Header */}
              ...
              <div className="flex flex-wrap gap-2">
                {exp.tech.map((t) => (
                  <span
                    key={t}
                    className="px-2.5 py-1 text-[10px] font-semibold font-mono uppercase bg-zinc-900 border border-white/5 text-zinc-400 rounded hover:text-white hover:border-primary/20 transition-all"
                  >
                    {t}
                  </span>
                ))}
